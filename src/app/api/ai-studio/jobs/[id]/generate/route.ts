...
function createPrompt(
  ...
  const backgroundInstruction = transparent
    ...
    : [
        ...
        "Use only a subtle and realistic contact shadow.",
      ].join(" ");
  ...
  return [
    ...
    "Preserve exact color, material, fabric texture, knit pattern, stitching, seams, ribbing, cuffs, collar, buttons, zippers, pockets, labels, logos and prints.",
    "Preserve the real cut, proportions, sleeve length, body length and silhouette.",
    "Do not redesign, restyle, simplify, embellish or invent product details.",
    "Remove the original background, mannequin, body form, hands, hangers, clips, supports, furniture and unrelated objects.",
    "Present one garment only, centered, front-facing, upright and neatly arranged.",
    "Correct only minor perspective distortion, uneven lighting and accidental wrinkles.",
    "Retain natural fabric depth and construction.",
...
async function prepareSourceImage(
  ...
  const sourceMimeType =
    asString(row.source_mime_type) || "image/png";
  ...
  if (!isHeicMimeType(sourceMimeType)) {
    ...
    if (!supported.has(sourceMimeType)) {
      throw new ApiError(
  ...
  await supabase
    .from("ai_studio_jobs")
    .update({
      source_conversion_status: "PROCESSING",
      source_conversion_error: null,
      updated_at: new Date().toISOString(),
    })
    .eq("organization_id", organizationId)
    .eq("id", jobId);
  ...
  try {
    ...
    const heicConvert = imported.default;
    ...
    const convertedBytes = Buffer.from(
      convertedArrayBuffer,
    );

    if (convertedBytes.length === 0) {
      throw new Error(
        "De geconverteerde PNG is leeg.",
      );
    }
    ...
    const { error: uploadError } =
      await supabase.storage
        .from(AI_STUDIO_BUCKET)
        .upload(processedPath, convertedBytes, {
    ...
    await supabase
      .from("ai_studio_jobs")
      .update({
        processed_source_bucket: AI_STUDIO_BUCKET,
        processed_source_path: processedPath,
        processed_source_mime_type: "image/png",
        processed_source_file_size:
          convertedBytes.length,
        source_conversion_status: "COMPLETED",
        source_conversion_error: null,
        updated_at: new Date().toISOString(),
      })
      .eq("organization_id", organizationId)
    ...
    const message =
      conversionError instanceof Error
        ...
        : "HEIC-conversie is mislukt.";

    await supabase
      .from("ai_studio_jobs")
      .update({
        source_conversion_status: "FAILED",
        source_conversion_error: message.slice(0, 2000),
        updated_at: new Date().toISOString(),
      })
      .eq("organization_id", organizationId)
      .eq("id", jobId);
    ...
    throw new ApiError(
      `De iPhone-foto kon niet naar PNG worden geconverteerd: ${message}`,
      500,
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const imageArrayBuffer =
      preparedSource.bytes.buffer.slice(
        preparedSource.bytes.byteOffset,
        preparedSource.bytes.byteOffset +
          preparedSource.bytes.byteLength,
      ) as ArrayBuffer;
    ...
    if (!openAiResponse.ok) {
      ...
      throw new ApiError(
        requestId
          ? `${apiMessage} Request-ID: ${requestId}`
          : apiMessage,
    ...
    const sourceUrl = await createSignedUrl(
      supabase,
      asString(completedJob.source_path),
    );

    const processedSourceUrl = await createSignedUrl(
      supabase,
      asString(
        completedJob.processed_source_path,
      ),
    );
    ...
    return NextResponse.json({
      ...
      sourceUrl,
      processedSourceUrl,
      sourceConverted: preparedSource.converted,
      resultUrl,
      resultPath,
      completedAt,
    });
