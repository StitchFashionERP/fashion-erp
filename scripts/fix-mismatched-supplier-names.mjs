...
const CORRECTIONS = {
  "Mode Sofia": "Mode Sofia srl",
  "Wolf": "Wolf Group s.r.l.",
  "Noir": "Noir Abbigliamento",
  "arnanknit": "Aranknit srl",
};

for (const [staleName, correctName] of Object.entries(CORRECTIONS)) {
  const { data: products, error } = await supabase
    .from("products")
    .select("id, product_code, name, profile")
    .eq("profile->>supplier", staleName);

  if (error) throw error;

  if (products.length === 0) {
    console.log(`No products found for "${staleName}" (already fixed?)`);
    continue;
  }
  ...
  for (const product of products) {
    ...
    console.log(
      `Updated ${product.product_code} (${product.name}): "${staleName}" -> "${correctName}"`,
    );
  }
}
